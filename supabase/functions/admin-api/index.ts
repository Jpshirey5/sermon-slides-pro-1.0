...
const overviewRevenue = async (ctx: AdminContext, body: any) => {
  const days = Number(body?.days) || 30;
  const { start, end } = buildDailyBuckets(days);
  const result = await computeRevenueMetrics({
    stripe: getStripe(),
  ...
  return { rangeDays: normalizeOverviewRangeDays(days), ...result };
};

const overviewUsage = async (ctx: AdminContext, body: any) => {
  const days = Number(body?.days) || 30;
  const { start, end } = buildDailyBuckets(days);
  const excludedAccountIds = await getInternalAccountIds(ctx.supabaseAdmin);
  const usage = await computeUsageMetrics({
    supabaseAdmin: ctx.supabaseAdmin,
    startIso: start.toISOString(),
    endIso: end.toISOString(),
    excludedAccountIds,
  });
  return { rangeDays: normalizeOverviewRangeDays(days), ...usage };
};
