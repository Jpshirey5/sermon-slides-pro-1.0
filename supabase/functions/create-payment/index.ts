...
// Pay-per-sermon price ID from Stripe
...
serve(async (req) => {
  ...
  try {
    ...
    const { sermonId, returnUrl } = body;

    logStep("Request body parsed", { sermonId, returnUrl });
    ...
    // Create one-time payment session (guest checkout - no auth required)
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${origin}${redirectPath}?payment=success&session_id={CHECKOUT_SESSION_ID}`,
